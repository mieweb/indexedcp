...
if (typeof window === 'undefined') {
  ...
  // const cliMode = process.env.INDEXEDCP_CLI_MODE;
  // const useFileSystemStorage = cliMode === 'true';

  // if (useFileSystemStorage) {
  //   // Legacy filesystem-db for CLI mode
  //   const { openFileSystemDB } = require('./filesystem-db');
  //   openDB = openFileSystemDB;
  //   isFileSystem = true;
  // } else
  if (testMode) {
    // Testing: use fake-indexeddb (in-memory, fast)
    require('fake-indexeddb/auto');
    const { openDB: idbOpenDB } = require('idb');
    openDB = idbOpenDB;
  } else {
    // Production: use indexeddbshim (SQLite-backed, persistent)
    // Note: Requires node-sqlite3 to be installed and compiled
    try {
      const setGlobalVars = require('indexeddbshim');
      // Use home directory for persistent storage to avoid CWD issues
      const dbPath = path.join(os.homedir(), '.indexcp', 'db');
      // Ensure directory exists before IndexedDBShim tries to create database
      if (!fs.existsSync(dbPath)) {
        fs.mkdirSync(dbPath, { recursive: true });
      }
      setGlobalVars(global, { checkOrigin: false, databaseBasePath: dbPath });
      const { openDB: idbOpenDB } = require('idb');
      openDB = idbOpenDB;
    } catch (err) {
      // Fallback to fake-indexeddb if indexeddbshim fails to load
      console.warn('[IndexedCP] IndexedDBShim not available, falling back to in-memory storage. Install node-sqlite3 for persistence.');
      require('fake-indexeddb/auto');
      const { openDB: idbOpenDB } = require('idb');
      openDB = idbOpenDB;
    }
