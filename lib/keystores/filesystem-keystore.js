...
class FileSystemKeyStore extends BaseKeyStore {
  ...
  async initialize() {
    try {
      ...
      console.log(`✓ Filesystem keystore initialized: ${this.keyStorePath}`);
    } catch (error) {
      console.error('Failed to initialize filesystem keystore:', error);
      throw error;
    }
  }
  ...
  async save(kid, keyData) {
    try {
      ...
      console.log(`🔑 Persisted key to filesystem: ${kid}`);
    } catch (error) {
      console.error(`Failed to save key ${kid}:`, error);
      throw error;
    }
  }
  ...
  async delete(kid) {
    try {
      ...
      console.log(`🗑️  Deleted key from filesystem: ${kid}`);
      return true;
    } catch (error) {
      if (error.code === 'ENOENT') {
        return false; // Already deleted or never existed
      }
      console.error(`Failed to delete key ${kid}:`, error);
      throw error;
    }
  }
