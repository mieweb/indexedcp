...
class MemoryKeyStore extends BaseKeyStore {
  ...
  async initialize() {
    if (this.ephemeral) {
      console.log('⚠️  Memory keystore initialized (EPHEMERAL - keys lost on restart!)');
    } else {
      console.log('✓ Memory keystore initialized');
    }
  }

  async save(kid, keyData) {
    this.keys.set(kid, { ...keyData });
    console.log(`🔑 Saved key to memory: ${kid}`);
  }
  ...
  async delete(kid) {
    ...
    if (existed) {
      console.log(`🗑️  Deleted key from memory: ${kid}`);
    }
  ...
  async close() {
    if (this.ephemeral) {
      this.keys.clear();
      console.log('✓ Memory keystore cleared');
    }
  }
