...
class MongoDBKeyStore extends BaseKeyStore {
  ...
  async initialize() {
    try {
      ...
      console.log(`✓ MongoDB keystore initialized: ${this.dbName}.${this.collectionName}`);
    } catch (error) {
      console.error('Failed to initialize MongoDB keystore:', error);
      throw error;
    }
  }
  ...
  async save(kid, keyData) {
    try {
      ...
      console.log(`🔑 Persisted key to MongoDB: ${kid}`);
    } catch (error) {
      console.error(`Failed to save key ${kid} to MongoDB:`, error);
      throw error;
    }
  }
  ...
  async load(kid) {
    try {
      ...
      console.error(`Failed to load key ${kid} from MongoDB:`, error);
      throw error;
    }
  }
  ...
  async loadAll() {
    try {
      ...
      console.error('Failed to load all keys from MongoDB:', error);
      return [];
    }
  }
  ...
  async delete(kid) {
    try {
      ...
      if (result.deletedCount > 0) {
        console.log(`🗑️  Deleted key from MongoDB: ${kid}`);
        return true;
      }
      
      return false; // Key not found
    } catch (error) {
      console.error(`Failed to delete key ${kid} from MongoDB:`, error);
      throw error;
    }
  }
  ...
  async exists(kid) {
    try {
      ...
      console.error(`Failed to check existence of key ${kid}:`, error);
      return false;
    }
  }
  ...
  async close() {
    ...
    console.log('✓ MongoDB keystore closed (client still managed by application)');
  }
  ...
  async cleanup(maxAge) {
    try {
      ...
      console.log(`🧹 Cleaned up ${result.deletedCount} old keys from MongoDB`);
  ...
  async find(query) {
    try {
      ...
      console.error('Failed to find keys:', error);
      return [];
    }
  }
}
